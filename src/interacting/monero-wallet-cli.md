...
Кошелёк подключается к [полному узлу](/interacting/monerod-reference), чтобы просканировать выходы ваших транзакций, а также отправить ваши транзакции в сеть.
...
| Опция            | Описание
|------------------|------------------------------------------------------------------------------------------------
| (отсутствует)    | По умолчанию кошелёк выбирает [основную сеть](/infrastructure/networks).
| `--stagenet`     | Запускает кошелёк в [отладочной сети](/infrastructure/networks). Следует помнить, что демон также запускается в `--stagenet`.
| `--testnet`      | Запускает кошелёк в [тестовой сети](/infrastructure/networks). Следует помнить, что демон также запускается в `--testnet`.
...
При совершении всех нелокальных операций кошелёк зависит от полного узла. Следующие опции определяют, как подсоединиться к [`monerod`](/interacting/monerod-reference):
...
| Опция                    | Описание
|--------------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `--config-file <arg>`    | Указывает полный путь к [конфигурационному файлу](/interacting/monero-config-file). Следует отметить, что это должен быть отдельный конфигурационный файл, используемый `monerod`, поскольку эти инструменты принимают различные наборы опций.
