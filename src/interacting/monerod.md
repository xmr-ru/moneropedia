...
| `--version`         | Показывает версию `monerod` в stdout. Пример: <br />`Monero 'Oxygen Orion' (v0.17.1.8-release)`
| `--os-version`      | Показывает отметку времени сборки и целевую операционную систему. Пример: <br />`OS: Linux #65-Ubuntu SMP Thu Dec 10 12:01:51 UTC 2020 5.4.0-59-generic`.
...
| `--pidfile`         | Полный путь к PID файлу. Работает только с `--detach`. Пример: <br />`./monerod --detach --pidfile=/run/monero/monerod.pid`
...
| `--p2p-bind-port-ipv6` | TCP-порт для прослушивания сетевых p2p соединений. По умолчанию совпадает с портом IPv4 для данного типа сети.
...
| `--out-peers`          | Задаёт максимальное количество исходящих соединений с другими узлами. Значение, используемое по умолчанию, — `12`. Значение `-1` указывает на ошибку в коде.
...
| `--max-connections-per-ip` | Максимальное количество соединений, разрешенных с одного IP-адреса.
...
| Опция                  | Описание
|------------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `--tx-proxy`           | Отправляйте свои транзакции через прокси-сервер SOCKS5 (Tor or I2P) в следующем формате:<br />`<network-type>,<socks-ip:port>[,max_connections][,disable_noise]` <br /><br />Например:<br />`./monerod --tx-proxy "tx-proxy=tor,127.0.0.1:9050,16"`<br /><br />Это было сделано, чтобы упростить проведение транзакций через Tor (не прибегая к использованию библиотеки torsocks) и в то же самое время сделать возможным публикацию блоков через клирнет (в то время как использование torsocks влияло абсолютно на всё). <br /><br />Команда `disable_noise` отключает Dandelion++, что ускоряет трансляцию транзакций (не рекомендуется к использованию). <br /><br />Следует отметить, что пересылаемые транзакции (инициатором которых не является подключённый кошелёк) по-прежнему будут транслироваться через клирнет.<br /><br />Для добавления «луковых» p2p узлов (сидов) требуется ручной ввод нескольких команд `--add-peer` (см. Добавление луковых раздающих узлов в p2p сеть Monero, а также это [руководство](/infrastructure/tor-onion-p2p-seed-nodes и [соответствующий коммит](https://github.com/monero-project/monero/pull/6021)).
| `--anonymous-inbound`  | Обеспечивает возможность подключения анонимных входящих соединений к вашему «луковому» P2P интерфейсу. Формат: <br />`<hidden-service-address>,<[bind-ip:]port>[,max_connections]`<br /><br />Пример:<br />`./monerod --anonymous-inbound "rveahdfho7wo4b2m.onion:18083,127.0.0.1:18083,100"`.<br /><br />Очевидно, что сначала нужно настроить сервис в вашей конфигурации Tor. См. [руководство](https://github.com/monero-project/monero/blob/master/docs/ANONYMITY_NETWORKS.md#p2p-commands).
...
| `--proxy`              | Передача данных по сети через прокси-сервер. Работает с любым сервисом, поддерживающим SOCKS4, включая Tor, i2p, коммерческие VPN и прокси-сервисы. Реализация поддержки SOCKS5 ожидается в будущем. Включение этой функции направляет весь трафик через указанный прокси-сервер. Может использоваться в сочетании с командой `--tx-proxy`. В этом случае транслируемые транзакции, источником которых является подключённый кошелёк (или кошельки), передают трафик через Tor или i2p, в зависимости от того, что будет указано в `--tx-proxy`, а весь остальной трафик будет проходить через прокси-сервер SOCKS. При этом используется следующий формат:<br />`<socks-ip:port>`
...
| `--rpc-restricted-bind-ipv6-address` | Работка с IPv6 через ограниченную версию API. Для создания открытого узла ограниченная версия API может быть сделана публичной. По умолчанию используется настройка `::1` (localhost). Чтобы обеспечить совместимость со всеми интерфейсами, следует изменить её на `::`.
...
| `--disable-rpc-ban`             | Отключает добавление хосов в черный список при возникновении ошибок RPC. Поиошает избежать запрещению monerod трафика, исходящего от демона Tor.
| `rpc-payment-address`           | Ограничить RPC клиентам, осуществляющим микроплатежи на указанный адрес.
| `rpc-payment-difficulty`        | Ограничить RPC клиентам, осуществляющим микроплатежи определенного уровня сложности (в тысячах).
| `rpc-payment-credits`           | Ограничить RPC клиентам, осуществляющим микроплатежи, что приводит к тому, что количество входящих транзакций на один платёж исчисляется сотнями.
| `rpc-payment-allow-free-loopback` | Разрешить свободный доступ с «петлевого» адреса (то есть, локального хоста).
...
| Опция                              | Описание
|------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `--keep-alt-blocks`                | Сохраняет альтернативные блоки при перезагрузке. Может помочь при рассмотрении возможности реорганизации и т. д. [Коммит](https://github.com/monero-project/monero/pull/5524). Исследовательский проект [noncesense.org](https://noncesense-research-lab.github.io/).
...
| Опция                 | Описание
|-----------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `--ban-list`          | Указать файл с черным списком IP-адресов, один IP-адрес на строку. Эта функция была добавлена в декабре 2020 / январе 2021 года как экстренная мера на случай проведения масштабной DDoS-атаки на p2p сеть Monero. Например <br />`./monerod --ban-list=block.txt`. А здесь можно найти популярный файл [block.txt](https://gui.xmr.pm/files/block.txt).<br /><br />Следует избегать **статистической блокировки** каких-либо IP-адресов, если в этом нет острой необходимости. При блокировке IP-адресов зачастую исключаются самые уязвимые пользователи, которые вынуждены пользоваться Tor или другими анонимными сетями.
| `--enable-dns-blocklist` | Подобно `--ban-list`, но вместо него, статистический файл использует динамический черный список IP-адресов, доступный во входящих данных DNS TXT. Содержание списка контролируется контрибьюторами Monero. **Не рекомендуется** к использованию за исключением действительно экстренных ситуаций.
...
| `--zmq-pub`           | Адрес публичного ZMQ - `tcp://ip:port` или `ipc://path`.
...
| `version`                          | Информация о версии ПО. Пример: <br />`Monero 'Boron Butterfly' (v0.14.0.0-release)`
...
| `ban <IP> [<seconds>]`             | Блокирует определённый `<IP>` (IP-адрес) на заданное количество `<seconds>` (секунд). По умолчанию время блокировки составляет 24 часа. Пример: <br />`./monerod ban 187.63.135.161`.
| `unban <IP>`                       | Снятие блокировки с заданного `<IP>` (IP-адреса).
| `bans`                             | Показывает заблокированные на данный момент `<IP>` (IP-адреса). Пример: <br />`187.63.135.161 banned for 86397 seconds`.
